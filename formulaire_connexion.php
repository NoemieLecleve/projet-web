...
<title>Ebuy</title>
	...
	<body>
		...
		<main>			
			...
			<!-- FORMULAIRE -->
			<!-- ********** -->
			<form method="post" action="connexion.php" class="container">
			    <fieldset class="col-6 offset-md-2">
			  	  <legend class="inscription text-center">Connexion</legend>
	                  	...
	                  	<div class="cadre">
			                ...
			                <div class="form-group">
			                    ...
			                    <input type="password" class="form-control" name="password" id="MotdePasse" placeholder="Mot de passe" required>
				               	 ...
				               	 <br>
				                <button type="submit" class="btn btn-outline-info" name="inscription">Se connecter</button>
				            </div>
				            <?php
								if (!empty($err_connexion)) {
									echo '<ul class="erreur">'."\n";
									foreach($err_connexion as $err) {
										echo
										' <li>'.$err.'</li>'."\n"; 
									}
									echo '</ul>';
							...
							<div class="text-center mt-2">
								<p>Pas encore de compte ? <a href="formulaire_inscription.php">Inscrivez-vous</a></p>
							</div>
			     </fieldset>
			</form>
		</main>
